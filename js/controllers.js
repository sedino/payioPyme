...
.controller('ResumCtrl', function($scope,$stateParams,NgTableParams,$filter,socket,$state,pagoService,$timeout,userData, Cajas,ventas) {
 ...
 //########################tabla de retiro, ingresos, balance ###################
  ...
  //##############donut data##############
  $scope.chartPieData = $scope.createPieChartData;
  $scope.createPieChartData = function (salesData) {
            var chartData = {
                labels: [],
                data: []
            };
...
.controller('InventCtrl', function($scope,$timeout,Cajas,$ionicPopup, $ionicModal,$stateParams,$state,$filter,userData,socket,invent,detalle, NgTableParams) {
  if (!userData.datos.userId) {
    $state.go('login');
  }
